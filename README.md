...
- 전국 날씨 통계 페이지 (평균 기온, 최고/최저 기온 지역, 관측 도시 수, 즐겨찾기 수)
- 기타 외부 API 연동 (adviceslip.com 오늘의 한마디)
...
검색 기능을 만들 때 cities.filter(city => city.name.includes(query))처럼 배열 메서드만으로 검색 로직을 짤 수 있다는 게 편했다. filter()는 원본 배열을 건드리지 않고
새 배열을 반환해서 반응형 상태를 직접 변형할 걱정이 없었고, includes()로 부분 문자열 매칭을 쓰니 정규식 없이도 검색이 됐다. 이 필터링 로직을 computed 안에 넣어두니
검색어(searchQuery)가 바뀔 때마다 자동으로 다시 계산돼서 별도 감시 로직을 안 짜도 됐는데, 그러면서 watch랑 watchEffect의 차이도 헷갈렸다. watch는 감시할 대상(source)을
명시적으로 지정해야 하고 그 값이 바뀔 때만 실행되는 반면, watchEffect는 함수 안에서 참조하는 반응형 값을 알아서 추적해서 그중 하나라도 바뀌면 실행되고, 심지어 마운트
시점에 한 번은 무조건 즉시 실행된다는 게 처음엔 낯설었다. **- Weather Composition hands on -**
...
.gitignore의 *.local 패턴으로 가리는 작업을 따로 해야 했다. 그리고 "OpenWeatherMap 안에서 API를 하나 더 붙이는 것"과 "완전히 다른 회사의 API를 붙이는 것"이
요구사항상 별개 항목이라는 걸 뒤늦게 구분해서, adviceslip.com이라는 날씨랑 무관한 API를 따로 하나 더 붙였다(오늘의 한마디). 이 API는 실패해도 날씨 표시엔 지장이 없어야
하니까 fetchRandomAdvice().catch(() => {})처럼 에러를 조용히 무시하는 처리를 여기서 처음 의식적으로 써봤다. **- Weather Axios hands on -**
...
안 이어진다는 걸 Leaflet 소스코드를 직접 읽어보고서야 알았다. 추측만으로 고치려다 두 번 실패해서, 라이브러리 내부 동작은 결국 소스를 읽어야 확실해진다는 걸 느꼈다.
아이콘도 원래는 이모지를 그대로 쓰고 있었는데, UI 라이브러리를 붙인 김에 전부 @element-plus/icons-vue 컴포넌트로 교체했다. 이모지는 문자열이라 el-alert의 title prop이나
el-descriptions-item의 label prop처럼 문자열만 받는 자리에는 그냥 넣으면 됐는데, 실제 아이콘 컴포넌트로 바꾸려니 그런 문자열 전용 prop 자리에는 컴포넌트를 못 넣어서 슬롯이 있는지부터
확인해야 했다. 그 과정에서 color prop으로 아이콘마다 의미에 맞는 색(맑음은 노랑, 비는 파랑, 즐겨찾기는 금색 등)을 입힐 수 있다는 것도 알게 됐다. **- Weather UI Library hands on -**
